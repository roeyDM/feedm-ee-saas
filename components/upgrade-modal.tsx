...
import { Zap, Check, X, ArrowRight, ShieldCheck } from "lucide-react";
...
export function UpgradeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative flex flex-col items-center text-center animate-in zoom-in-95 duration-200 border border-zinc-200">
        
        {/* Close Button */}
        <button
          onClick={() => onOpenChange(false)}
          className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-700 bg-zinc-100 hover:bg-zinc-200 rounded-full p-2 transition cursor-pointer"
        >
          <X className="h-4 w-4" />
        </button>

        {/* Icon Header */}
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-tr from-amber-400 via-emerald-500 to-teal-600 text-white shadow-xl shadow-emerald-500/20 mb-4">
        ...
        <Link href="/pricing" className="w-full">
          <Button className="w-full h-12 rounded-xl text-xs font-black bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/25 cursor-pointer gap-2 transition hover:scale-[1.01]">
            <span>Start 7-Day Free Trial ($7/mo)</span>
        ...
        <div className="mt-3 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-zinc-500">
          ...
          <span>No credit card required for trial • Cancel anytime</span>
        </div>
      </div>
    </div>
  );
